...
class SGD_NS_Overshoot(Optimizer):
    ...
    def _generate_heavy_tailed_noise(self, x, alpha=3.0, complex=False, clamp_value=10.0, eps=1e-8):
        u = torch.rand_like(x)
        base = u.pow(-1.0 / alpha) - 1

        if complex:
            real = torch.rand_like(x.real)
            imag = torch.rand_like(x.imag)
            direction = torch.complex(real, imag)
            direction = direction / (direction.abs() + eps)
        else:
            direction = torch.sign(torch.rand_like(x))

        noise = direction * base

        if complex:
            mag = LA.vector_norm(noise)
    ...
    def _compute_cosine_similarity(self, grad, m):
        grad = grad.view(-1)
        m = m.view(-1)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if "exp_avg_sq" not in state:
                    ...
                    exp_avg_sq.mul_(rms_beta).addcmul_(grad, grad, alpha=1.0 - rms_beta)
                ...
                # ----------------------------------------   
                noise_decay = max((self._noise_decay_steps - self._global_step) / self._noise_decay_steps, 0.2)
                if grad.ndim == 4:
                    Gf = torch.fft.rfft2(grad, norm='ortho')

                    noise = self._generate_heavy_tailed_noise(Gf, complex=True)
                    noise = noise / (noise.abs().mean() + eps)
                    Gf_noisy = Gf + noise * self._noise_scale * noise_decay

                    grad = torch.irfft2(Gf_noisy, s=grad.shape[-2:], norm='ortho')

                elif grad.ndim == 2:
                    noise = self._generate_heavy_tailed_noise(grad, complex=False)
                    noise = noise / (noise.abs().mean() + eps)

                    grad = grad + noise * self._noise_scale * noise_decay
